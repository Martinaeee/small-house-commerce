...
- sessionStorage 不可用（隐私模式）：REVIEW ORDER 后确认页读不到 draft → 按 §3.4 重定向回表单页。**注（终审裁定 Ruling #7）：§3.4 的按钮迁移已使「降级为原单步不破坏下单」在该场景下不可达（storage 受限浏览器会陷入表单↔确认重定向循环，无法下单）——本节降级承诺视为被 §3.4 取代；storage 受限浏览器罕见、无数据损坏风险，暂不提供回退提交路径。**容错与 A 批同款 try/catch。
